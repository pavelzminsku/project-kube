...
def yc_login():
    user = os.getenv("USER")
    profiles = subprocess.getoutput('/home/{user}/yandex-cloud/bin/yc config profile list')
    logging.debug(f"Profiles: {profiles}")
    if not profiles or not 'otus' in profiles:
        error = 0
        error += os.system('/home/{user}/yandex-cloud/bin/yc config profile create otus')
        ...
        if not error:
            ...
            error += os.system('/home/{user}/yandex-cloud/bin/yc config set service-account-key temp_key.json')
            logging.debug(f"Adding key errors: {error}")
            error += os.system('rm temp_key.json')
            logging.debug(f"Removing file errors: {error}")
            if not error:
                error += os.system('/home/{user}/yandex-cloud/bin/yc config set cloud-id $CLOUD')
                logging.debug(f"Adding cloud errors: {error}")
                error += os.system('/home/{user}/yandex-cloud/bin/yc config set folder-id $FOLDER')
...
def create_cluster(cluster_name: str, nodes: int):
    user = os.getenv("USER")
    clusters = json.loads(subprocess.getoutput('/home/{user}/yandex-cloud/bin/yc managed-kubernetes cluster list --format json'))
    ...
    if not cluster_exists:
        ...
        creation = json.loads(subprocess.getoutput(f'/home/{user}/yandex-cloud/bin/yc managed-kubernetes cluster create \
